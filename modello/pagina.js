...
(DATA.roster || []).forEach(r => {
  ...
  // role_counts (quante partite per reparto, mostrato nella scheda giocatore) si calcola
  // piu' sotto con groupCountsSorted(), dopo che i gruppi di roles.json sono caricati: deve
  // contare il reparto reale (con le correzioni a mano), non l'etichetta EA grezza per
  // partita. Vedi GROUP_COUNTS_BY_NAME.
...
// ---- Quante partite per reparto reale, per la scheda giocatore ----
// Il badge "Attacco N partite / Centrocampo M partite" deve contare le STESSE partite
// della classifica per reparto (computeGroupScores piu' sotto), non l'etichetta EA
// grezza per partita: altrimenti i due numeri non tornano mai. Un COC come Pesix_97 ha
// 122 partite "forward"/"midfielder" secondo EA, ma solo 92 contano davvero come
// Attaccanti dopo le correzioni a mano di roles.json (eccezioni_partita). Segnalato da
// Peppe il 05/09/2026: i due numeri devono coincidere.
const GROUP_COUNTS_BY_NAME = (function(){
  const m = new Map();
  (DATA.matches || []).forEach(mt => {
    (DATA.matchPlayers[mt.match_id] || []).forEach(p => {
      if(!p.pos) return;
      const gruppo = ROLE_EXCEPTIONS[mt.match_id + "|" + p.player_name]
                  || groupForMatch(p.player_name, p.pos);
      if(!gruppo) return;
      if(!m.has(p.player_name)) m.set(p.player_name, {});
      const c = m.get(p.player_name);
      c[gruppo] = (c[gruppo] || 0) + 1;
    });
  });
  return m;
})();

function groupCountsSorted(name){
  const c = GROUP_COUNTS_BY_NAME.get(name);
  return c ? Object.entries(c).sort((a,b) => b[1] - a[1]) : [];
}

// Etichette brevi per il badge: stesso stile di ROLE_LABELS ma per reparto reale
...
(DATA.roster || []).forEach(r => {
  ...
  // Stesso conteggio della classifica per reparto: il badge nella scheda giocatore e
  // "Partite nel ruolo" mostrano sempre lo stesso numero.
  const sortedGruppi = groupCountsSorted(r.player_name);
...
function computeFormScores(windowSize){
  ...
  // Conteggio grezzo delle presenze nella finestra, PRIMA del filtro sulla soglia minima.
  // Serve a chi resta sotto soglia: senza questo la scheda non sa dire "ne hai 3" e puo'
  // solo dire "nessuna", che e' falso per chi ne ha 1-3 (bug analogo gia' trovato e corretto
  // nella scheda giocatore, vedi commento piu' sotto su role_from_matches).
  const gamesByPlayer = new Map([...agg.entries()].map(([name, a]) => [name, a.games]));
  ...
  const formMap = pool.length === 0 ? new Map() : new Map(pool.map(p => [p.name, {
    ...p,
...
function computeBlendedScores(windowSize, weight){
  const { formMap: form, gamesByPlayer } = computeFormScores(windowSize);
  const punteggi = POWER_SCORES.map(s => {
    ...
    // EA non manda i tentativi grezzi di carriera per il club, solo realizzati e
    // percentuale: tentativiDiCarriera() li stima invertendo la percentuale, e a 0%
    // realizzati e percentuale coincidono a zero - la stima non sa piu' distinguere
    // "zero tentativi davvero" da "alcuni tentativi, tutti falliti". La finestra recente
    // e' pero' un SOTTOINSIEME della carriera e i suoi tentativi sono contati per davvero
    // dalle partite archiviate: se superano la stima, quel numero vero diventa il minimo.
    // Trovato il 19/09/2026 aprendo la rosa di FC 27 a inizio stagione: prima, con la
    // soglia dei 30 a svuotarla, questo ramo non veniva mai eseguito su un campione cosi'
    // piccolo da avere davvero uno 0%.
    const tCarStimato = tentativiDiCarriera(g);
    const tCar = {
      passaggi:  Math.max(tCarStimato.passaggi,  hasForm ? f.tentPassaggi : 0),
      contrasti: Math.max(tCarStimato.contrasti, hasForm ? f.tentContrasti : 0),
      tiro:      Math.max(tCarStimato.tiro,      hasForm ? f.tentTiro : 0),
    };
    const carriera = {
      rating: g.rating_ave, contrib: (g.goals + g.assists) / gp, motm: g.man_of_the_match / gp,
      tech: efficienzaTecnica(g.pass_success_rate, g.tackle_success_rate, g.shot_success_rate, g.gruppo, tCar),
      disc: g.red_cards / gp,
    ...
    return {
      ...
      // Presenze REALI nella finestra, anche sotto la soglia minima: serve a chi resta
      // fuori classifica per dire quante ne ha davvero invece di dire sempre "nessuna".
      formGamesInWindow: gamesByPlayer.get(s.r.player_name) || 0,
...
// L'ascolto sta sulla RIGA delle intestazioni, non sulle singole celle.
//
// Le celle vengono ricostruite a ogni disegno (le colonne aggiunte compaiono e spariscono),
// quindi un ascoltatore attaccato a una cella muore con lei. Il primo tentativo riattaccava
// gli ascoltatori dopo ogni ricostruzione, saltando quelle gia' collegate con un attributo
// `data-collegata` - ma le celle si ricostruiscono da `outerHTML`, che quell'attributo se lo
// porta dietro: rinascevano tutte marcate come collegate, e nessuna lo era piu'.
// Risultato: le intestazioni non rispondevano piu' al clic. Segnalato subito, il 01/09/2026.
//
// Un ascoltatore solo sulla riga - che non viene mai sostituita - non ha questo problema e
// non ha stato da tenere in ordine.
(function ascoltaIntestazioni(){
  const testa = document.getElementById("rosterHead");
  if(!testa) return;
  testa.addEventListener("click", (e) => {
    const th = e.target.closest("th");
    if(th && th.dataset.key) ordinaPer(th.dataset.key);
  });
})();
  ...
  function draw(){
    ...
    tbody.innerHTML = rows.map((s) => {
      if(s.fuoriClassifica){
        // "Nessuna" e' vero solo a 0. Da 1 a FORM_MIN_APPS-1 il giocatore le partite
        // le ha, solo non abbastanza: dirlo in modo diverso evita di sembrare un dato
        // mancante quando invece e' sotto soglia (segnalato il 03/09/2026 su gio05596,
        // 3 presenze contro le 4 richieste).
        const n = s.formGamesInWindow;
        const motivoBreve = n === 0
          ? "nessuna partita archiviata"
          : `solo ${n} partit${n === 1 ? "a" : "e"} archiviat${n === 1 ? "a" : "e"} (ne servono ${FORM_MIN_APPS})`;
        const motivoTitolo = n === 0
          ? "Nessuna partita archiviata nella finestra scelta"
          : `${n} partit${n === 1 ? "a" : "e"} archiviat${n === 1 ? "a" : "e"} nella finestra scelta, ne servono ${FORM_MIN_APPS}`;
        return `
        <tr style="opacity:.6;">
          <td data-label="#"><span class="opp-tag" title="${motivoTitolo}">—</span></td>
          ...
          <td data-label="Indice" class="lb-value" colspan="2" style="font-size:12px; color:var(--muted);">
            non valutabile sulla forma: ${motivoBreve}</td>
...
(function renderDiagnosi(){
  ...
  tbody.innerHTML = VOCI.map(v => {
    ...
    let diff = "";
    if(v.quanto !== null){
      let testo = null, rilevante = false, segno = 0;
      if(v.gol){
        const d = c - a;
        ...
        const diffAssoluta = c - a;
        const scritto = v.f(Math.abs(diffAssoluta));
        testo = scritto === v.f(0) ? "uguale" : `${diffAssoluta > 0 ? "+" : "−"}${scritto}`;
        rilevante = a ? Math.abs(100 * diffAssoluta / Math.abs(a)) >= 8 : diffAssoluta !== 0;
        segno = diffAssoluta;
      }
      if(testo !== null){
        // Su richiesta di Peppe (04/09/2026): solo i delta negativi risaltano in rosso,
        // quelli positivi restano nel colore del testo normale, cosi' il rosso significa
        // sempre "peggio nelle sconfitte" e non semplicemente "e' cambiato qualcosa".
        const colore = !rilevante ? "var(--muted)" : (segno < 0 ? "var(--loss)" : "var(--text)");
        diff = `<span style="color:${colore};">${rilevante ? testo : "quasi uguale"}</span>`;
      }
    }
    return `<tr>
      <td data-label="Indicatore">${v.lab}</td>
      <td data-label="Vittoria" class="diag-win">${v.f(a)}</td>
      <td data-label="Pareggio">${b == null ? "—" : v.f(b)}</td>
      <td data-label="Sconfitta" class="diag-loss">${v.f(c)}</td>
...
function schedaDaPartite(name){
  ...
  return {
    ...
    // Stesso criterio della rosa: reparto reale (con le correzioni di roles.json), non
    // l'etichetta EA grezza per partita. Vedi GROUP_COUNTS_BY_NAME.
    role_counts: Object.fromEntries(groupCountsSorted(name)),
      ...
      <div>
        ...
        <div class="role-split">
          ${r.role_from_matches
            ...
            ? Object.entries(r.role_counts).sort((a,b) => b[1]-a[1]).map(([gruppo, n]) =>
                `<span class="rs"><b>${GROUP_SHORT_LABELS[gruppo] || gruppo}</b> ${n} ${n === 1 ? "partita" : "partite"}</span>`).join("")
