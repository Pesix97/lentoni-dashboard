    ...
    return `
      <details style="margin-top:8px;">
        ...
        <div style="margin:8px 0 2px; padding-left:12px; border-left:2px solid var(--panel-2,rgba(255,255,255,.10));">
          ${righe.map(r => `
            <div style="display:flex; justify-content:space-between; gap:10px; padding:5px 0; font-size:12px;">
              <span>${r.p.eti}
                <span style="color:var(--muted); font-size:10.5px;">
                  — ${r.p.su}, pesa ${(100 * PESI_TECNICA[r.p.k]).toFixed(0)}%</span>
              </span>
              <span style="white-space:nowrap;">
                <span style="color:var(--muted);">${pc(r.a)} contro ${pc(r.b)}</span>
                <strong style="color:${r.diff >= 0 ? "var(--ok,#4ade80)" : "var(--accent)"}; margin-left:8px;">
                  ${r.diff >= 0 ? "+" : "−"}${Math.abs(r.diff).toFixed(1)}</strong>
              </span>
            </div>`).join("")}
