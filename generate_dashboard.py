...
<section id="premi">
  <h2>Premi &amp; record della rosa <span class="h2-sub">— scegli quali mostrare · solo giocatori con almeno __MIN_GAMES__ partite</span></h2>
  <div class="panel" style="margin-bottom:12px;">
    <div class="filter-bar" id="awardFilters"></div>
  </div>
...
<section id="statsdivertenti">
  <h2>Stats divertenti <span class="h2-sub">— almeno una per ogni giocatore, generata dalle statistiche reali, con affetto</span></h2>
  <div class="panel">
    <div class="roast-list" id="roastList"></div>
  </div>
</section>
...
<section id="sessioni">
  <h2>Analisi delle serate <span class="h2-sub">— cosa succede col passare delle partite, e chi rende con chi</span></h2>
  <div class="panel" style="margin-bottom:12px;">
    <div style="font-size:12px; color:var(--muted); line-height:1.5;">
      Le partite archiviate vengono raggruppate in <strong style="color:var(--text);">serate</strong>: due partite appartengono
      alla stessa sessione se tra l'una e l'altra passano meno di 3 ore. Nessun dato nuovo, solo gli orari
      che erano già nell'archivio.
      <br><br>
      I punti sono calcolati come in campionato: 3 per la vittoria, 1 per il pareggio, 0 per la sconfitta.
    </div>
  </div>

  <h3 style="margin:18px 0 10px; font-size:16px;">Rendimento col passare della serata</h3>
  <div class="panel" style="margin-bottom:12px;">
    <div id="sessionSummary" style="font-size:13px; line-height:1.6;"></div>
  </div>
  <div class="panel" style="margin-bottom:16px;">
    <div class="table-wrap">
      <table id="sessionPosTable" class="responsive-table">
        <thead><tr>
          <th>Partita n.</th><th>Volte giocata</th><th>Gol fatti</th><th>Gol subiti</th>
          <th>Punti a partita</th><th>Andamento</th>
        </tr></thead>
        <tbody></tbody>
      </table>
    </div>
    <div style="font-size:12px; color:var(--muted); margin-top:10px;">
      Il campione si assottiglia man mano che si va avanti: poche serate arrivano alla settima partita,
      quindi le ultime righe vanno lette con prudenza. La stanchezza è solo una delle spiegazioni
      possibili — a fine serata cambiano anche gli avversari e può cambiare chi resta in campo.
    </div>
  </div>

  <h3 style="margin:22px 0 10px; font-size:16px;">Le serate</h3>
  <div class="panel" style="margin-bottom:16px;">
    <div class="table-wrap">
      <table id="sessionListTable" class="responsive-table">
        <thead><tr><th>Serata</th><th>Partite</th><th>Bilancio</th><th>Gol</th><th>Punti a partita</th></tr></thead>
        <tbody></tbody>
      </table>
    </div>
  </div>

  <h3 style="margin:22px 0 10px; font-size:16px;">Chi rende con chi</h3>
  <div class="panel" style="margin-bottom:12px;">
    <div style="font-size:12px; color:var(--muted); line-height:1.5;">
      Per ogni coppia di giocatori si contano solo le partite in cui erano <strong style="color:var(--text);">entrambi
      in campo</strong>, e si guarda come è andata alla squadra. È una fotografia, non una spiegazione:
      due giocatori che compaiono spesso insieme condividono anche tutto il resto — gli altri compagni,
      gli avversari, l'ora della notte. Serve a farsi venire domande, non a dare colpe.
    </div>
  </div>
  <div class="panel">
    <div class="lb-controls" style="margin-bottom:10px;">
      <span style="font-size:12px; color:var(--muted);">Minimo partite insieme:</span>
      <div class="filter-bar" id="pairMinFilters" style="display:inline-flex;"></div>
    </div>
    <div class="table-wrap">
      <table id="pairsTable" class="responsive-table">
        <thead><tr>
          <th>Coppia</th><th>Insieme</th><th>Punti a partita</th><th>Gol fatti</th><th>Gol subiti</th><th>Differenza</th>
        </tr></thead>
        <tbody></tbody>
      </table>
    </div>
...
// ---- Awards & records (selezionabili) ----
const DEFAULT_AWARD_KEYS = ["goals", "assists", "rating_ave", "games_played", "red_cards", "man_of_the_match", "pass_success_rate", "tackle_success_rate", "contribution_pg"];
let selectedAwardKeys = new Set(DEFAULT_AWARD_KEYS);

function renderAwardFilters(){
  const el = document.getElementById("awardFilters");
  el.innerHTML = STAT_DEFS.map(def => `<span class="filter-btn" data-key="${def.key}">${def.icon} ${def.label}</span>`).join("");
  el.querySelectorAll(".filter-btn").forEach(btn => {
    btn.classList.toggle("active", selectedAwardKeys.has(btn.dataset.key));
    btn.addEventListener("click", () => {
      const key = btn.dataset.key;
      if(selectedAwardKeys.has(key)) selectedAwardKeys.delete(key);
      else selectedAwardKeys.add(key);
...
function renderAwards(){
  const el = document.getElementById("awardGrid");
  const roster = DATA.roster || [];
  if(roster.length === 0){
    el.innerHTML = `<div class="empty">Nessun giocatore con almeno ${LEADERBOARD_MIN_GAMES} partite ancora.</div>`;
    return;
  }
  if(selectedAwardKeys.size === 0){
    el.innerHTML = '<div class="empty">Nessun premio selezionato — scegline almeno uno qui sopra.</div>';
    return;
  }
  const cards = [];
  STAT_DEFS.filter(def => selectedAwardKeys.has(def.key)).forEach(def => {
    const winner = roster.reduce((best, r) => {
      const v = statValue(def, r);
      const bv = best ? statValue(def, best) : -Infinity;
      return v > bv ? r : best;
    }, null);
    if(!winner) return;
    const value = statValue(def, winner);
    cards.push(`
      <div class="award-card">
        <div class="icon">${def.icon}</div>
        <div class="title">${def.awardTitle}</div>
        <div class="name">${winner.player_name}</div>
        <div class="stat">${fmtStatValue(def, value)} ${def.unit} · ${winner.games_played} partite</div>
      </div>
    `);
  });
  el.innerHTML = cards.join("") || '<div class="empty">Nessun dato sufficiente per i premi selezionati.</div>';
}
renderAwardFilters();
renderAwards();
...
// ---- Analisi delle serate ----
// Due partite appartengono alla stessa sessione se distano meno di questo intervallo.
// Le serate osservate durano 1-3 ore con 15-25 minuti tra una partita e l'altra, quindi
// tre ore separano bene una sessione dall'altra senza spezzare quelle lunghe.
const SESSION_GAP_MS = 3 * 3600 * 1000;

function punti(m){
  if(m.goals_for > m.goals_against) return 3;
  if(m.goals_for === m.goals_against) return 1;
  return 0;
}

function buildSessions(){
  const ordinate = [...(DATA.matches || [])]
    .filter(m => m.played_at)
    .sort((a, b) => new Date(a.played_at) - new Date(b.played_at));
  const sessioni = [];
  ordinate.forEach(m => {
    const corrente = sessioni[sessioni.length - 1];
    const precedente = corrente && corrente[corrente.length - 1];
    if(precedente && (new Date(m.played_at) - new Date(precedente.played_at)) <= SESSION_GAP_MS){
      corrente.push(m);
    } else {
      sessioni.push([m]);
    }
...
(function renderSessioni(){
  const posBody = document.querySelector("#sessionPosTable tbody");
  if(!posBody) return;
  const sessioni = buildSessions();

  // --- rendimento in base alla posizione nella serata ---
  const perPosizione = new Map();
  sessioni.forEach(s => s.forEach((m, i) => {
    const k = i + 1;
    if(!perPosizione.has(k)) perPosizione.set(k, { n:0, gf:0, ga:0, pt:0 });
    const a = perPosizione.get(k);
    a.n++; a.gf += m.goals_for || 0; a.ga += m.goals_against || 0; a.pt += punti(m);
  }));
  const posizioni = [...perPosizione.entries()].sort((a, b) => a[0] - b[0]);
  const maxPunti = Math.max(1, ...posizioni.map(([, a]) => a.pt / a.n));

  posBody.innerHTML = posizioni.map(([k, a]) => {
    const pp = a.pt / a.n;
    const largh = Math.round((pp / 3) * 100);
    const scarso = a.n < 3;
    return `<tr>
      <td data-label="Partita n.">${k}ª</td>
      <td data-label="Volte giocata">${a.n}${scarso ? ` <span class="pos-badge" title="Campione troppo piccolo per trarne conclusioni">poche</span>` : ""}</td>
      <td data-label="Gol fatti">${(a.gf / a.n).toFixed(2)}</td>
      <td data-label="Gol subiti">${(a.ga / a.n).toFixed(2)}</td>
      <td data-label="Punti a partita"><strong>${pp.toFixed(2)}</strong></td>
      <td data-label="Andamento">
        <div style="background:var(--panel-2,rgba(255,255,255,.06)); border-radius:4px; height:8px; width:100%; min-width:60px;">
          <div style="width:${largh}%; height:8px; border-radius:4px; background:var(--accent);"></div>
        </div>
      </td>
    </tr>`;
  }).join("") || `<tr><td colspan="6" class="empty">Nessuna partita archiviata.</td></tr>`;

  // --- confronto inizio / fine serata ---
  const somma = (filtro) => posizioni.filter(([k]) => filtro(k))
    .reduce((acc, [, a]) => ({ n:acc.n+a.n, gf:acc.gf+a.gf, ga:acc.ga+a.ga, pt:acc.pt+a.pt }), {n:0,gf:0,ga:0,pt:0});
  const inizio = somma(k => k <= 3);
  const fine = somma(k => k >= 5);
  const el = document.getElementById("sessionSummary");
  if(inizio.n && fine.n){
    const diff = (fine.pt / fine.n) - (inizio.pt / inizio.n);
    const verso = diff < -0.1 ? "cala" : (diff > 0.1 ? "migliora" : "resta stabile");
    el.innerHTML =
      `<strong>Prime 3 partite della serata</strong> (${inizio.n} giocate): `
      + `<strong style="color:var(--accent);">${(inizio.pt/inizio.n).toFixed(2)}</strong> punti a partita, `
      + `${(inizio.gf/inizio.n).toFixed(2)} gol fatti e ${(inizio.ga/inizio.n).toFixed(2)} subiti.<br>`
      + `<strong>Dalla quinta in poi</strong> (${fine.n} giocate): `
      + `<strong style="color:var(--accent);">${(fine.pt/fine.n).toFixed(2)}</strong> punti a partita, `
      + `${(fine.gf/fine.n).toFixed(2)} gol fatti e ${(fine.ga/fine.n).toFixed(2)} subiti.<br><br>`
      + `Il rendimento ${verso} di <strong>${Math.abs(diff).toFixed(2)}</strong> punti a partita. `
      + (Math.abs((fine.gf/fine.n) - (inizio.gf/inizio.n)) < 0.5
          ? `I gol fatti restano praticamente identici: quello che cambia non è l'attacco.` : ``);
  } else {
    el.innerHTML = `<span class="empty">Servono più serate archiviate per confrontare inizio e fine.</span>`;
  }

  // --- elenco delle serate ---
  const listBody = document.querySelector("#sessionListTable tbody");
  listBody.innerHTML = [...sessioni].reverse().map(s => {
    const v = s.filter(m => m.goals_for > m.goals_against).length;
    const p = s.filter(m => m.goals_for === m.goals_against).length;
    const sc = s.length - v - p;
    const pt = s.reduce((a, m) => a + punti(m), 0);
    const gf = s.reduce((a, m) => a + (m.goals_for || 0), 0);
    const ga = s.reduce((a, m) => a + (m.goals_against || 0), 0);
    const d = new Date(s[0].played_at);
    return `<tr>
      <td data-label="Serata">${d.toLocaleDateString("it-IT", { day:"2-digit", month:"short" })} · ${d.toLocaleTimeString("it-IT", { hour:"2-digit", minute:"2-digit" })}</td>
      <td data-label="Partite">${s.length}</td>
      <td data-label="Bilancio">${v}V ${p}P ${sc}S</td>
      <td data-label="Gol">${gf}-${ga}</td>
      <td data-label="Punti a partita">${(pt / s.length).toFixed(2)}</td>
    </tr>`;
  }).join("") || `<tr><td colspan="5" class="empty">Nessuna serata archiviata.</td></tr>`;
})();

// ---- Chi rende con chi ----
(function renderCoppie(){
  const body = document.querySelector("#pairsTable tbody");
  const filtri = document.getElementById("pairMinFilters");
  if(!body) return;

  const inRosa = new Set((DATA.roster || []).map(r => r.player_name));
  const risultato = new Map((DATA.matches || []).map(m => [m.match_id, m]));
  const coppie = new Map();

  (DATA.matches || []).forEach(m => {
    // Set: la stessa persona non deve poter formare coppia con se stessa se per
    // qualche motivo comparisse due volte nella stessa partita.
    const presenti = [...new Set((DATA.matchPlayers[m.match_id] || [])
      .map(p => p.player_name).filter(n => inRosa.has(n)))].sort();
    for(let i = 0; i < presenti.length; i++){
      for(let j = i + 1; j < presenti.length; j++){
        const k = presenti[i] + " | " + presenti[j];
        if(!coppie.has(k)) coppie.set(k, { a:presenti[i], b:presenti[j], n:0, pt:0, gf:0, ga:0 });
        const c = coppie.get(k);
        c.n++; c.pt += punti(m); c.gf += m.goals_for || 0; c.ga += m.goals_against || 0;
      }
    }
  });

  const OPZIONI = [5, 10, 15, 20];
  let minimo = 10;

  function disegna(){
    filtri.innerHTML = OPZIONI.map(n =>
      `<span class="filter-btn ${n === minimo ? "active" : ""}" data-min="${n}">${n}</span>`).join("");
    filtri.querySelectorAll(".filter-btn").forEach(b =>
      b.addEventListener("click", () => { minimo = Number(b.dataset.min); disegna(); }));

    const righe = [...coppie.values()].filter(c => c.n >= minimo)
      .sort((x, y) => (y.pt / y.n) - (x.pt / x.n));
    body.innerHTML = righe.length ? righe.map(c => {
      const pp = c.pt / c.n, diff = (c.gf - c.ga) / c.n;
      return `<tr>
        <td data-label="Coppia"><span class="player-link" data-player="${c.a}">${c.a}</span> + <span class="player-link" data-player="${c.b}">${c.b}</span></td>
        <td data-label="Insieme">${c.n}</td>
        <td data-label="Punti a partita"><strong>${pp.toFixed(2)}</strong></td>
        <td data-label="Gol fatti">${(c.gf / c.n).toFixed(2)}</td>
        <td data-label="Gol subiti">${(c.ga / c.n).toFixed(2)}</td>
        <td data-label="Differenza" style="color:${diff >= 0 ? "var(--ok,#4ade80)" : "var(--accent)"};">${diff >= 0 ? "+" : ""}${diff.toFixed(2)}</td>
      </tr>`;
    }).join("") : `<tr><td colspan="6" class="empty">Nessuna coppia con almeno ${minimo} partite insieme.</td></tr>`;
  }
...
// ---- Stats divertenti: almeno una per ogni giocatore ----
// Per ciascun giocatore si calcola quale statistica lo rende più "particolare"
// rispetto alla media della rosa (deviazione standard più alta in valore assoluto),
// e si genera una frase in tono scherzoso su quella statistica. Così ogni membro
// della rosa riceve almeno una battuta, basata su un dato reale e non a caso.
(function renderFunStats(){
  const el = document.getElementById("roastList");
  const roster = DATA.roster || [];
  if(roster.length === 0){
    el.innerHTML = `<div class="empty">Nessun giocatore con almeno ${LEADERBOARD_MIN_GAMES} partite ancora.</div>`;
    return;
  ...
  // Metriche candidate: compute() estrae il valore per un giocatore.
  // direction "high" = viene commentato solo se sopra media, "low" = solo se sotto media, "both" = in entrambi i casi.
  const METRICS = [
    { key: "rating_ave", direction: "both", compute: r => r.rating_ave,
      high: (r,v) => [`👑`, `<b>${r.player_name}</b> vola con una media di ${v.toFixed(2)}, tra le più alte della rosa. Gli altri dovrebbero chiedere lezioni private.`],
      low:  (r,v) => [`📉`, `<b>${r.player_name}</b> gioca con una media di ${v.toFixed(2)}, sotto la media squadra. Magari la prossima volta passa la palla prima.`] },
    { key: "goals_pg", direction: "both", compute: r => r.games_played ? r.goals / r.games_played : 0,
      high: (r,v) => [`⚽`, `<b>${r.player_name}</b> macina gol a raffica: ${v.toFixed(2)} a partita. Un vizio incurabile.`],
      low:  (r,v) => [`🙈`, `<b>${r.player_name}</b> con ${r.goals} gol in ${r.games_played} partite: il gol per lui resta un concetto astratto.`] },
    { key: "assists_pg", direction: "both", compute: r => r.games_played ? r.assists / r.games_played : 0,
      high: (r,v) => [`🎯`, `<b>${r.player_name}</b> è un assist-man seriale: ${v.toFixed(2)} a partita. Generosità o paura di tirare?`],
      low:  (r,v) => [`🧍`, `<b>${r.player_name}</b>: ${r.assists} assist in ${r.games_played} partite. Il pallone esiste anche per i compagni.`] },
    { key: "pass_success_rate", direction: "both", compute: r => r.pass_success_rate,
      high: (r,v) => [`📡`, `<b>${r.player_name}</b> è chirurgico nei passaggi: ${v}% di precisione. GPS del pallone perfettamente calibrato.`],
      low:  (r,v) => [`🎲`, `<b>${r.player_name}</b> passa il pallone giusto col ${v}% di successo. Un po' a casaccio, diciamocelo.`] },
    { key: "tackle_success_rate", direction: "both", compute: r => r.tackle_success_rate,
      high: (r,v) => [`🛡️`, `<b>${r.player_name}</b> è un muro in difesa: ${v}% di contrasti vinti. Passare da lì è vietato.`],
      low:  (r,v) => [`🚧`, `<b>${r.player_name}</b> vince solo il ${v}% dei contrasti. I contrasti li perde più che vincerli.`] },
    { key: "shot_success_rate", direction: "both", compute: r => r.shot_success_rate,
      high: (r,v) => [`🎯`, `<b>${r.player_name}</b> è un cecchino sotto porta: ${v}% di precisione al tiro.`],
      low:  (r,v) => [`💨`, `<b>${r.player_name}</b> tira in porta col ${v}% di precisione. Le tribune ringraziano per i palloni regalati.`] },
    { key: "red_cards_pg", direction: "high", compute: r => r.games_played ? r.red_cards / r.games_played : 0,
      high: (r,v) => [`🟥`, `<b>${r.player_name}</b> ha collezionato ${r.red_cards} cartellini rossi. L'arbitro ormai lo saluta per nome.`] },
    { key: "motm_rate", direction: "both", compute: r => r.games_played ? r.man_of_the_match / r.games_played : 0,
      high: (r,v) => [`⭐`, `<b>${r.player_name}</b> è MVP seriale: ${r.man_of_the_match} MOTM in ${r.games_played} partite. Un abbonamento al premio.`],
      low:  (r,v) => [`🎭`, `<b>${r.player_name}</b>: ${r.man_of_the_match} MOTM in ${r.games_played} partite. Il migliore in campo lo lascia sempre agli altri.`] },
    { key: "win_rate", direction: "both", compute: r => r.win_rate,
      high: (r,v) => [`🍀`, `<b>${r.player_name}</b> porta fortuna: ${v}% di vittorie con lui in campo.`],
      low:  (r,v) => [`⛈️`, `<b>${r.player_name}</b> vince solo il ${v}% delle partite giocate. Jella o coincidenza?`] },
    { key: "games_played", direction: "high", compute: r => r.games_played,
      high: (r,v) => [`🏋️`, `<b>${r.player_name}</b> è un habitué: ${v} partite giocate, più di chiunque altro. La maglia gli sta cucita addosso.`] },
  ];

  function mean(arr){ return arr.reduce((s,v)=>s+v,0) / arr.length; }
  function std(arr, m){ return Math.sqrt(arr.reduce((s,v)=>s+(v-m)**2,0) / arr.length) || 0; }

  const stats = METRICS.map(metric => {
    const values = roster.map(metric.compute);
    const m = mean(values);
    const s = std(values, m);
    return { metric, values, mean: m, std: s };
  });

  const funStats = roster.map((r, i) => {
    let best = null;
    stats.forEach(({metric, values, mean: m, std: s}) => {
      const v = values[i];
      const z = s > 0 ? (v - m) / s : 0;
      let scoreZ = null, dir = null;
      if((metric.direction === "high" || metric.direction === "both") && z > 0){ scoreZ = z; dir = "high"; }
      if(metric.direction === "both" && z < 0 && -z > (scoreZ ?? -Infinity)){ scoreZ = -z; dir = "low"; }
      if(scoreZ !== null && (!best || scoreZ > best.scoreZ)){
        best = { metric, dir, v, scoreZ };
      }
    });
    if(!best){
      return [`⚖️`, `<b>${r.player_name}</b> gioca senza infamia e senza lode: nella media della squadra in tutto.`];
    }
    return best.dir === "high" ? best.metric.high(r, best.v) : best.metric.low(r, best.v);
  });

  el.innerHTML = funStats.map(([emoji, text]) => `<div class="roast-item"><span class="emoji">${emoji}</span><span>${text}</span></div>`).join("");
})();

// ---- Head to head tra giocatori della rosa ----
(function renderH2H(){
...
const PAGE_MAP = {
  ...
  riepilogo: "riepilogo", sessioni: "sessioni", avversari: "avversari", partite: "partite",
};
const PAGES = [
  ...
  { key: "sessioni", icon: "🌙", label: "Analisi serate" },
