...
(function renderDiagnosi(){
  ...
  const VOCI = [
    ...
    { k: "scarto",      lab: "Scarto reti medio",        f: x => (x > 0 ? "+" : "") + x.toFixed(1), gol: true },
  ...
  tbody.innerHTML = VOCI.map(v => {
    ...
    if(v.quanto !== null){
      ...
      if(v.gol){
        const d = c - a;
        testo = `${d > 0 ? "+" : ""}${d.toFixed(1)} gol`;
        rilevante = Math.abs(d) >= 0.5;
      } else if(v.perc){
        const d = Math.round(c - a);
        testo = `${d > 0 ? "+" : ""}${d} ${Math.abs(d) === 1 ? "punto" : "punti"}`;
        rilevante = Math.abs(d) >= 5;
      } else if(a){
        const d = 100 * (c - a) / Math.abs(a);
        testo = `${d > 0 ? "+" : ""}${Math.round(d)}%`;
        rilevante = Math.abs(d) >= 8;
      }
      if(testo !== null){
        diff = rilevante
  ...
  VOCI.filter(v => CONFRONTABILI.includes(v.k)).forEach(v => {
    ...
    const testo = punti
      ? `${d > 0 ? "+" : ""}${Math.round(d)} ${Math.abs(Math.round(d)) === 1 ? "punto" : "punti"}`
      : `${d > 0 ? "+" : ""}${Math.round(d)}%`;
...
(function renderOsservatore(){
  ...
  function scartoDi(d, suo, base){
    ...
    return d.perc ? { valore: suo - base, testo: `${p > 0 ? "+" : ""}${p} ${Math.abs(p) === 1 ? "punto" : "punti"}` }
  ...
  const posizioneInSerata = {};
  (DATA.serate || []).forEach(s => s.matchIds.forEach((id, i) => posizioneInSerata[id] = i + 1));
  ...
  function curiosita(g){
    ...
    // Tiene la serata o cala.
    const inizio = righe.filter(r => (posizioneInSerata[r.match_id] || 9) <= 2).map(r => r.rating);
    const coda = righe.filter(r => (posizioneInSerata[r.match_id] || 0) >= 5).map(r => r.rating);
    if(inizio.length >= 5 && coda.length >= 5){
      const d = media(coda) - media(inizio);
      scheda("Come regge la serata",
        Math.abs(d) < 0.15
          ? `Uguale dall'inizio alla fine: <strong>${media(inizio).toFixed(2)}</strong> nelle prime due partite,
             <strong>${media(coda).toFixed(2)}</strong> dalla quinta in poi.`
          : `<strong>${media(inizio).toFixed(2)}</strong> nelle prime due partite della serata,
             <strong>${media(coda).toFixed(2)}</strong> dalla quinta in poi: ${d > 0 ? "cresce" : "cala"} di
             ${Math.abs(d).toFixed(2)}.`,
        `${seg(inizio.length)} a inizio serata, ${seg(coda.length)} in coda`);
    }
